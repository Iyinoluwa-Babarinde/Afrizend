...
export default function EmployerJobsPage() {
    ...
    return (<div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
        <div>
          <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))" }}>Jobs & Contracts</h1>
          <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem", marginTop: 4 }}>Manage your active projects and review applicants</p>
        </div>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", overflowX: "auto", paddingBottom: "0.25rem" }}>
          {["ALL", "OPEN", "IN_PROGRESS", "COMPLETED"].map((f) => (<button key={f} onClick={() => setFilter(f)} style={{
                ...
                border: filter === f ? "1px solid hsl(217 91% 55% / 0.5)" : "1px solid hsl(var(--surface-4))",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        {filteredJobs.map((job) => {
            ...
            return (<div key={job.id} className="card card-hover" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <div style={{ flex: 1, paddingRight: "1rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
                    ...
                    {job.isBackend && (<span style={{ fontSize: "0.65rem", fontWeight: 700, color: "hsl(var(--success))", background: "hsl(145 65% 42% / 0.12)", border: "1px solid hsl(145 65% 42% / 0.25)", padding: "0.1rem 0.4rem", borderRadius: 4, textTransform: "uppercase" }}>
                        Live
                      </span>)}
                    ...
                    {job.status === "OPEN" && job.isBackend && applicantCount > 0 && (
                      <button
                        ...
                        style={{ display: "flex", alignItems: "center", gap: "0.3rem", fontSize: "0.68rem", fontWeight: 700, color: "hsl(var(--primary-dark))", background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.3)", padding: "0.15rem 0.5rem", borderRadius: 4, cursor: "pointer" }}
                      >
                  ...
                  <div style={{ display: "flex", gap: "1rem", fontSize: "0.8rem", color: "hsl(var(--text-2))", marginBottom: "0.75rem", flexWrap: "wrap" }}>
                    ...
                    <span>Budget: <strong style={{ color: "hsl(var(--text))" }}>{formatCurrency(job.budget)}</strong></span>
                  ...
                  <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                    {job.skills?.map((s) => (<span key={s} style={{ fontSize: "0.7rem", color: "hsl(var(--text-2))", background: "hsl(var(--surface-3))", padding: "0.2rem 0.5rem", borderRadius: 4 }}>{s}</span>))}
                  </div>
                </div>
              ...
              {/* Escrow progress bar */}
              {job.status !== "OPEN" && job.escrow && (<div style={{ borderTop: "1px solid hsl(var(--border))", paddingTop: "1rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.4rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                      <Lock size={12} color="hsl(145 65% 50%)"/>
                      <span style={{ fontSize: "0.75rem", color: "hsl(var(--text-2))" }}>Escrow Progress</span>
                    </div>
                    <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "hsl(var(--success))" }}>
                      {formatCurrency(job.escrow.released)} released of {formatCurrency(job.escrow.total)}
                    </span>
                  </div>
                  <div style={{ height: 8, background: "hsl(var(--surface-3))", borderRadius: 99, overflow: "hidden" }}>
        ...
        {filteredJobs.length === 0 && !loadingBackend && (<div className="card" style={{ textAlign: "center", padding: "3rem 1rem" }}>
            ...
            <h3 className="font-heading" style={{ fontSize: "1.1rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>No jobs yet</h3>
            <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem", marginBottom: "1.5rem" }}>Post your first job and let Gemini AI structure it into verified milestones.</p>
            <Link to="/dashboard/employer/jobs/new" className="btn btn-primary">
      ...
      {viewingApplicantsJob && (<div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }} className="animate-fade-in">
          <div className="card" style={{ width: "100%", maxWidth: 600, padding: "1.75rem", position: "relative", maxHeight: "85vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <button onClick={() => setViewingApplicantsJob(null)} style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", cursor: "pointer", color: "hsl(var(--text-2))" }}>
              <X size={20}/>
            </button>

            <div style={{ marginBottom: "1.25rem" }}>
              <h3 className="font-heading" style={{ fontSize: "1.2rem", color: "hsl(var(--text))", marginBottom: "0.25rem" }}>
                Applicants for "{viewingApplicantsJob.title}"
              </h3>
              <p style={{ fontSize: "0.82rem", color: "hsl(var(--text-2))" }}>
            ...
            {acceptSuccess ? (<div style={{ textAlign: "center", padding: "2rem" }}>
                ...
                <h4 className="font-heading" style={{ color: "hsl(var(--text))", fontSize: "1.1rem", marginBottom: "0.5rem" }}>Hired {acceptSuccess}!</h4>
                <p style={{ color: "hsl(var(--text-2))", fontSize: "0.85rem" }}>Redirecting to contract management…</p>
              </div>) : (<div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "1rem" }}>
                {loadingApplicants && (<div style={{ textAlign: "center", padding: "2rem", color: "hsl(var(--text-2))", fontSize: "0.85rem" }}>
                    Loading applicants…
                  </div>)}
                ...
                {!loadingApplicants && applicants.length === 0 && (<div style={{ textAlign: "center", padding: "2rem" }}>
                    ...
                    <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem" }}>No applicants yet. Share the job or wait for freelancers to apply.</p>
                  </div>)}
                ...
                {applicants.map(app => (
                  <div key={app.freelancer_id} className="card" style={{ display: "flex", gap: "1rem", alignItems: "flex-start", padding: "1rem" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 700, color: "hsl(var(--text))", fontSize: "0.9rem" }}>{app.name}</span>
                        ...
                        {app.hourly_rate > 0 && (
                          <span style={{ fontSize: "0.72rem", color: "hsl(var(--text-2))" }}>{formatCurrency(app.hourly_rate)}/hr</span>
                        )}
                      </div>
                      {app.cover_note && (
                      ...
                      <div style={{ display: "flex", gap: "0.3rem", flexWrap: "wrap" }}>
                        {(app.skills || []).slice(0, 4).map(s => (
                          <span key={s} style={{ fontSize: "0.68rem", color: "hsl(var(--text-2))", background: "hsl(var(--surface-3))", padding: "0.1rem 0.4rem", borderRadius: 4 }}>{s}</span>
                        ))}
                      </div>
                    </div>
                    <button
