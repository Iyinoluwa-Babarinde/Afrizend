...
export default function JobMatchesPage() {
  ...
  if (!job) {
    return (<div style={{ padding: "3rem", textAlign: "center" }}>
      ...
      <p style={{ color: "hsl(var(--text-2))" }}>Job not found.</p>
  ...
  return (<div style={{ padding: "1.5rem", maxWidth: 900, margin: "0 auto" }}>
    <Link to="/dashboard/employer/jobs" style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", color: "hsl(var(--text-2))", textDecoration: "none", fontSize: "0.875rem", marginBottom: "1.5rem" }}>
    ...
    <div style={{ marginBottom: "2rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
        ...
        <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))" }}>AI Candidate Matchmaker</h1>
        {liveFreelancers.length > 0 && (
          <span style={{ fontSize: "0.68rem", fontWeight: 700, color: "hsl(var(--success))", background: "hsl(145 65% 42% / 0.12)", border: "1px solid hsl(145 65% 42% / 0.25)", padding: "0.2rem 0.5rem", borderRadius: 4, display: "flex", alignItems: "center", gap: "0.25rem" }}>
            <Globe size={10} /> {liveFreelancers.length} Registered
          </span>
        )}
      </div>
      <p style={{ color: "hsl(var(--text-2))", fontSize: "0.9rem" }}>
    ...
    {hiredFreelancer ? (<div className="card text-center animate-fade-in" style={{ background: "hsl(145 65% 42% / 0.08)", borderColor: "hsl(145 65% 42% / 0.3)", padding: "3rem 2rem", borderRadius: 12 }}>
      ...
      <h2 className="font-heading" style={{ fontSize: "1.5rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>Hired Successfully!</h2>
      <p style={{ color: "hsl(var(--text-2))", fontSize: "0.95rem", maxWidth: 500, margin: "0 auto 1.5rem" }}>
        You have hired <strong>{hiredFreelancer.name}</strong>. The escrow contract is active and the first milestone has been structured. Redirecting you to manage the contract escrow...
      </p>
      ...
      {matches.map((freelancer) => {
        ...
        return (<div key={freelancer.id} className="card card-hover ai-match-card" style={{ display: "flex", flexDirection: "column", gap: "1.25rem", position: "relative" }}>
            ...
            <div style={{ display: "flex", gap: "1rem" }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <h3 className="font-heading" style={{ fontSize: "1.1rem", fontWeight: 700, color: "hsl(var(--text))" }}>{freelancer.name}</h3>
                ...
                <div style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", marginTop: 2 }}>{freelancer.title || "Freelancer"}</div>
              </div>
            </div>

            <div style={{ display: "flex", gap: "1.5rem", textAlign: "right" }}>
              <div>
                <div style={{ fontSize: "0.7rem", color: "hsl(var(--text-2))" }}>TRUST SCORE</div>
              ...
              <div>
                <div style={{ fontSize: "0.7rem", color: "hsl(var(--text-2))" }}>HOURLY RATE</div>
                <div style={{ fontSize: "0.95rem", fontWeight: 700, color: "hsl(var(--text))" }}>
          ...
          {/* Match rationale */}
          <div style={{ background: "hsl(var(--surface-3))", border: "1px solid hsl(var(--border))", borderRadius: 8, padding: "0.875rem 1rem", fontSize: "0.85rem", color: "hsl(var(--text-2))", display: "flex", alignItems: "flex-start", gap: "0.6rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
            {(freelancer.skills || []).map((skill) => {
              ...
              return (<span key={skill} style={{
                ...
                background: isMatched ? "hsl(145 65% 42% / 0.12)" : "hsl(var(--surface-3))",
                border: isMatched ? "1px solid hsl(145 65% 42% / 0.25)" : "1px solid hsl(var(--surface-4))",
