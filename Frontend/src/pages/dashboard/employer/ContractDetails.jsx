...
function StatusBadge({ status }) {
  const map = {
    LOCKED: { bg: "hsl(220 16% 14%)", color: "hsl(var(--text-3))", label: "Locked" },
    IN_PROGRESS: { bg: "hsl(217 91% 55% / 0.12)", color: "hsl(var(--primary-dark))", label: "In Progress" },
    ...
    APPROVED: { bg: "hsl(145 65% 42% / 0.12)", color: "hsl(var(--success))", label: "AI Verified - Pending Employer Approval" },
...
export default function ContractPage() {
  ...
  if (!job) {
    return (<div style={{ padding: "3rem", textAlign: "center" }}>
      <p style={{ color: "hsl(var(--text-2))" }}>Contract not found.</p>
  ...
  return (<div style={{ padding: "1.5rem", maxWidth: 900, margin: "0 auto" }}>
    <button onClick={() => navigate("/dashboard/employer/jobs")} style={{ display: "flex", alignItems: "center", gap: "0.4rem", background: "none", border: "none", color: "hsl(var(--text-2))", cursor: "pointer", fontSize: "0.875rem", marginBottom: "1.25rem", padding: 0 }}>
    ...
    <div style={{ marginBottom: "1.75rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
        <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))", letterSpacing: "-0.03em" }}>{job.title}</h1>
        <span style={{ fontSize: "0.72rem", fontWeight: 700, padding: "0.2rem 0.6rem", borderRadius: 6, background: "hsl(var(--primary) / 0.1)", color: "hsl(var(--primary-dark))", textTransform: "uppercase" }}>
          {job.status.replace("_", " ")}
        </span>
      </div>
      <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem" }}>
        Contractor: <strong style={{ color: "hsl(var(--text))" }}>{job.freelancerName}</strong> · Budget: <strong style={{ color: "hsl(var(--text))" }}>{formatCurrency(job.budget, job.currency || user?.currency || 'USD')}</strong>
      </p>
    </div>

    {/* Escrow Status Bar */}
    <div className="card" style={{ marginBottom: "1.5rem", background: job.escrow.status === "FUNDED" || job.escrow.status === "PARTIAL_RELEASE" ? "hsl(145 65% 42% / 0.05)" : "hsl(var(--surface-2))", border: `1px solid ${job.escrow.status === "FUNDED" || job.escrow.status === "PARTIAL_RELEASE" ? "hsl(145 65% 42% / 0.25)" : "hsl(var(--surface-4))"}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <div>
            <div style={{ fontSize: "0.9rem", fontWeight: 700, color: "hsl(var(--text))" }}>KORA Escrow</div>
            <div style={{ fontSize: "0.75rem", color: "hsl(var(--success))" }}>
              {job.escrow.KORATxId ? `TX: ${job.escrow.KORATxId}` : "Pending"}
            </div>
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div className="font-heading" style={{ fontSize: "1.2rem", fontWeight: 800, color: "hsl(var(--text))" }}>{formatCurrency(job.escrow.funded)}</div>
          <div style={{ fontSize: "0.72rem", color: "hsl(var(--text-2))" }}>locked on KORA network</div>
        </div>
      </div>
      <div style={{ height: 8, background: "hsl(var(--surface-4))", borderRadius: 99, overflow: "hidden", marginBottom: "0.6rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", color: "hsl(var(--text-2))" }}>
    ...
    {paidSuccess && (
      <div style={{ background: "hsl(145 65% 42% / 0.12)", border: "1px solid hsl(145 65% 42% / 0.4)", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "1rem" }}>
        ...
        <div>
          <div style={{ fontSize: "1rem", fontWeight: 700, color: "hsl(var(--success))" }}>✅ Payment Released Successfully!</div>
          <div style={{ fontSize: "0.85rem", color: "hsl(var(--text-2))", marginTop: "0.2rem" }}>
            {formatCurrency(paidSuccess.amount)} sent to freelancer's wallet
            {paidSuccess.freelancerNewBalance != null && <> · Freelancer new balance: <strong style={{ color: "hsl(var(--text))" }}>{formatCurrency(paidSuccess.freelancerNewBalance)}</strong></>}
          </div>
        </div>
      </div>
    )}
    ...
    {!job.downloadUnlocked && job.status !== "COMPLETED" && (<div style={{ background: "hsl(38 92% 50% / 0.08)", border: "1px solid hsl(38 92% 50% / 0.25)", borderRadius: 10, padding: "0.875rem 1.125rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
      ...
      <div>
        ...
        <div style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))" }}>Final deliverables are locked until all milestone payments are released. This protects the freelancer.</div>
      </div>
    </div>)}
    ...
    {job.downloadUnlocked && (<div style={{ background: "hsl(145 65% 42% / 0.08)", border: "1px solid hsl(145 65% 42% / 0.3)", borderRadius: 10, padding: "0.875rem 1.125rem", marginBottom: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}>
        ...
        <div style={{ fontSize: "0.9rem", fontWeight: 700, color: "hsl(var(--success))" }}>🎉 Download Unlocked — All payments released!</div>
      </div>
    ...
    {/* Milestones */}
    <div>
      <h2 className="font-heading" style={{ fontSize: "1.1rem", fontWeight: 700, color: "hsl(var(--text))", marginBottom: "1rem" }}>
        ...
        {job.milestones.map((milestone, i) => (<div key={milestone.id} className="card" style={{ position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ paddingLeft: "0.75rem" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", marginBottom: "0.75rem" }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
                  <span style={{ fontSize: "0.72rem", color: "hsl(var(--text-3))", textTransform: "uppercase", letterSpacing: "0.06em" }}>M{i + 1}</span>
                  <StatusBadge status={milestone.status} />
                </div>
                <h3 className="font-heading" style={{ fontSize: "1rem", fontWeight: 700, color: "hsl(var(--text))" }}>{milestone.title}</h3>
                <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", marginTop: "0.25rem" }}>{milestone.description}</p>
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div className="font-heading" style={{ fontSize: "1.15rem", fontWeight: 800, color: "hsl(var(--text))" }}>{formatCurrency(milestone.amount)}</div>
                {milestone.paidAt && <div style={{ fontSize: "0.7rem", color: "hsl(var(--success))" }}>Paid ✓</div>}
              </div>
            </div>
            ...
            {milestone.verificationResult && (
              <div style={{ background: "hsl(38 92% 50% / 0.06)", border: "1px solid hsl(38 92% 50% / 0.2)", borderRadius: 8, padding: "0.875rem", marginBottom: "0.75rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                  ...
                  <span style={{ fontSize: "0.78rem", fontWeight: 700, color: "hsl(var(--text-2))" }}>AI Review Report: <span style={{ color: "hsl(var(--success))" }}>Reviewed — Awaiting Your Approval</span></span>
                ...
                <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", lineHeight: 1.5 }}>{milestone.verificationResult.summary}</p>
                  ...
                  <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid hsl(var(--border))" }}>
                    {milestone.verificationResult.checklist?.map((item, ci) => (
                      <div key={ci} style={{ display: "flex", gap: "0.5rem", marginBottom: "0.4rem" }}>
                        ...
                        <span style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))" }}><strong style={{ color: item.passed ? "hsl(145 65% 55%)" : "hsl(0 84% 65%)" }}>{item.criterion}:</strong> {item.note}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {milestone.submittedFiles && milestone.submittedFiles.length > 0 && (
              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", marginBottom: "0.75rem" }}>
                {milestone.submittedFiles.map((f) => (
                  <span key={f} style={{ display: "flex", alignItems: "center", gap: "0.3rem", fontSize: "0.75rem", color: "hsl(var(--primary))", background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.3)", padding: "0.2rem 0.5rem", borderRadius: 4 }}>
                    <FileText size={11} /> {f}
                  </span>
                ))}
              </div>
            ...
            {(() => {
              ...
              return (
                <div style={{ marginTop: "0.5rem", paddingTop: "0.75rem", borderTop: "1px solid hsl(var(--border))" }}>
                  <div style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", marginBottom: "0.6rem" }}>
            ...
            {/* Paid confirmation */}
            {milestone.paidAt && (
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.5rem", fontSize: "0.82rem", color: "hsl(var(--success))" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginTop: "2rem" }}>
      ...
      <div className="card" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        ...
        {job.escrow.status === "PENDING" ? (
          ...
          <div style={{ background: "hsl(145 65% 42% / 0.08)", border: "1px solid hsl(145 65% 42% / 0.2)", borderRadius: 8, padding: "0.75rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            <span style={{ fontSize: "0.85rem", color: "hsl(var(--success))", fontWeight: 600 }}>Escrow Funded on KORA ✓</span>
          </div>
        )}
      </div>
      ...
      <div className="card" style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        <h3 className="font-heading" style={{ fontSize: "1.1rem", fontWeight: 700, color: "hsl(var(--text))" }}>Encrypted Collaboration</h3>

        {/* Chat Messages list */}
        <div style={{ flex: 1, minHeight: 120, background: "hsl(var(--surface-3))", border: "1px solid hsl(var(--border))", borderRadius: 8, padding: "0.75rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          <div style={{ fontSize: "0.72rem", color: "hsl(var(--text-3))", textAlign: "center", marginBottom: 4 }}>End-to-End Encrypted Tunnel</div>

          {chatMessages.map((msg, index) => (
            <div key={index} style={{ alignSelf: msg.sender === "You" ? "flex-end" : "flex-start", maxWidth: "80%" }}>
              <div style={{
                background: msg.sender === "You" ? "hsl(217 91% 55%)" : "hsl(var(--surface-3))",
                color: "hsl(var(--text))",
              ...
              <div style={{ fontSize: "0.65rem", color: "hsl(var(--text-3))", textAlign: msg.sender === "You" ? "right" : "left", marginTop: 2 }}>{msg.sender}</div>
            </div>
          ))}
        </div>
