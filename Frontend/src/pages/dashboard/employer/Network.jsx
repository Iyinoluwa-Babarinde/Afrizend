  ...
  return (<div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
    ...
    <div style={{ marginBottom: "2rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.4rem" }}>
        <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))" }}>Talent Network</h1>
        {liveFreelancers.length > 0 && (
          <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "hsl(var(--success))", background: "hsl(145 65% 42% / 0.12)", border: "1px solid hsl(145 65% 42% / 0.25)", padding: "0.2rem 0.5rem", borderRadius: 4, display: "flex", alignItems: "center", gap: "0.3rem" }}>
    ...
    <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
      ...
      <div style={{ display: "flex", gap: "0.5rem", overflowX: "auto", paddingBottom: "0.25rem" }}>
        {allSkills.slice(0, 8).map(skill => (<button key={skill} onClick={() => setSelectedSkill(skill)} style={{
          ...
          border: selectedSkill === skill ? "1px solid hsl(217 91% 55% / 0.5)" : "1px solid hsl(var(--surface-4))",
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.25rem" }}>
      {filteredFreelancers.map((f) => (<div key={f.id} className="card card-hover" style={{ display: "flex", flexDirection: "column", gap: "1.25rem", position: "relative" }}>
        ...
        {/* Live badge */}
        {f.isLive && (
          <div style={{ position: "absolute", top: "0.75rem", right: "0.75rem", fontSize: "0.62rem", fontWeight: 700, color: "hsl(var(--success))", background: "hsl(145 65% 42% / 0.15)", border: "1px solid hsl(145 65% 42% / 0.3)", padding: "0.15rem 0.4rem", borderRadius: 4, display: "flex", alignItems: "center", gap: "0.25rem" }}>
        ...
        <div style={{ display: "flex", gap: "1rem" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <h3 className="font-heading" style={{ fontSize: "1rem", fontWeight: 700, color: "hsl(var(--text))", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.name}</h3>
            <p style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.title || "Freelancer"}</p>
          </div>
        </div>

        {/* Bio */}
        {f.bio && (
          <p style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))", lineHeight: 1.5, margin: 0 }}>
            {f.bio.slice(0, 100)}{f.bio.length > 100 ? "…" : ""}
          </p>
        )}

        {/* Metrics */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", padding: "0.75rem", borderRadius: 8, background: "hsl(var(--surface-3))", border: "1px solid hsl(var(--border))" }}>
          <div>
            <span style={{ fontSize: "0.68rem", color: "hsl(var(--text-2))", textTransform: "uppercase" }}>Trust Score</span>
            <div style={{ fontSize: "0.85rem", color: "hsl(var(--text))", fontWeight: 700, display: "flex", alignItems: "center", gap: "0.25rem", marginTop: 2 }}>
              <Star size={13} fill="hsl(38 92% 50%)" color="hsl(38 92% 50%)" /> {trustScore(f)} / 100
            </div>
          </div>
          <div>
            <span style={{ fontSize: "0.68rem", color: "hsl(var(--text-2))", textTransform: "uppercase" }}>Hourly Rate</span>
            <div style={{ fontSize: "0.85rem", color: "hsl(var(--text))", fontWeight: 700, marginTop: 2 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
          {(f.skills || []).slice(0, 5).map(s => (<span key={s} style={{ fontSize: "0.7rem", color: "hsl(var(--text-2))", background: "hsl(var(--surface-3))", padding: "0.15rem 0.45rem", borderRadius: 4 }}>
            {s}
          </span>))}
        </div>

        {/* Actions */}
        <div style={{ display: "flex", gap: "0.5rem", borderTop: "1px solid hsl(var(--border))", paddingTop: "0.875rem", marginTop: "auto" }}>
    ...
    {filteredFreelancers.length === 0 && !loadingLive && (<div className="card text-center" style={{ padding: "4rem 2rem" }}>
      ...
      <h3 className="font-heading" style={{ fontSize: "1.1rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>No matches found</h3>
      <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem" }}>Try adjusting your search criteria or filter selections.</p>
    </div>)}

    {loadingLive && (<div style={{ textAlign: "center", padding: "2rem", color: "hsl(var(--text-2))", fontSize: "0.85rem" }}>
    ...
    {bookingConsultant && (<div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }} className="animate-fade-in">
      <div className="card glass-strong" style={{ width: "100%", maxWidth: 450, padding: "2rem", position: "relative" }}>
        <button onClick={() => setBookingConsultant(null)} style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", cursor: "pointer", color: "hsl(var(--text-2))" }}>
          <X size={20} />
        </button>
        ...
        {bookingSuccess ? (<div style={{ textAlign: "center", padding: "1rem 0" }} className="animate-fade-in">
          ...
          <h3 className="font-heading" style={{ fontSize: "1.2rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>Session Booked!</h3>
          <p style={{ color: "hsl(var(--text-2))", fontSize: "0.85rem" }}>Consultation scheduled and synced to your dashboard sessions.</p>
        </div>) : (<>
          <h3 className="font-heading" style={{ fontSize: "1.25rem", color: "hsl(var(--text))", marginBottom: "1rem" }}>Schedule Consultation</h3>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.5rem", padding: "0.75rem", borderRadius: 8, background: "hsl(var(--surface-3))" }}>
            ...
            <div>
              <div style={{ fontWeight: 700, color: "hsl(var(--text))", fontSize: "0.9rem" }}>{bookingConsultant.name}</div>
              <div style={{ fontSize: "0.75rem", color: "hsl(var(--text-2))" }}>{bookingConsultant.title}</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "1.5rem" }}>
            ...
            <div>
              <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "hsl(var(--text-2))", marginBottom: "0.4rem" }}>Duration</label>
    ...
    {chatConsultant && (<div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "100%", maxWidth: 380, zIndex: 999, background: "hsl(var(--surface-2))", borderLeft: "1px solid hsl(var(--border))", boxShadow: "var(--shadow-lg)", display: "flex", flexDirection: "column" }} className="animate-fade-in">

      {/* Header */}
      <div style={{ padding: "1.25rem", borderBottom: "1px solid hsl(var(--border))", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <div className="avatar" style={{ width: 36, height: 36, fontSize: "0.7rem", borderRadius: 8 }}>
            {chatConsultant.name.split(" ").map((n) => n[0]).join("")}
          </div>
          <div>
            <div style={{ fontWeight: 700, color: "hsl(var(--text))", fontSize: "0.9rem" }}>{chatConsultant.name}</div>
            <div style={{ fontSize: "0.72rem", color: "hsl(var(--success))", display: "flex", alignItems: "center", gap: "0.25rem" }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: "hsl(145 65% 50%)" }} /> Active Now
            </div>
          </div>
        </div>
        <button onClick={() => setChatConsultant(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "hsl(var(--text-2))" }}>
          <X size={20} />
        </button>
      </div>
        ...
        {(chatHistory[chatConsultant.id] || [
          ...
          return (<div key={i} style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
            <div style={{
              ...
              background: isMe ? "hsl(217 91% 55%)" : "hsl(var(--surface-3))",
              color: isMe ? "white" : "hsl(220 15% 85%)",
              border: isMe ? "none" : "1px solid hsl(var(--surface-4))",
              borderBottomRightRadius: isMe ? 2 : 12,
              borderBottomLeftRadius: isMe ? 12 : 2,
            }}>
              {msg.text}
            </div>
            <span style={{ fontSize: "0.68rem", color: "hsl(var(--text-3))", marginTop: 4 }}>{msg.sender}</span>
          </div>);
        })}
      </div>

      {/* Footer */}
      <div style={{ padding: "1.25rem", borderTop: "1px solid hsl(var(--border))", background: "hsl(var(--surface-2))" }}>
