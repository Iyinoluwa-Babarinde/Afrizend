...
export default function FreelancerDashboard() {
    ...
    const stats = [
        { label: "Total Earned", value: formatCurrency(user?.balance || 0, user?.currency || "NGN"), sub: "Available funds", icon: DollarSign, color: "green" },
    ...
    return (<div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "1.75rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
          ...
          <span style={{ fontSize: "0.78rem", color: "hsl(var(--success))", fontWeight: 600 }}>ILP Network Connected</span>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "1.25rem", alignItems: "start" }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            <h2 className="font-heading" style={{ fontSize: "1rem", fontWeight: 700, color: "hsl(var(--text))" }}>Active Contracts</h2>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
            {activeJobs.map(job => (<div key={job.id} className="card card-hover" style={{ cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                  <div>
                    <div className="font-heading" style={{ fontSize: "0.95rem", fontWeight: 700, color: "hsl(var(--text))", marginBottom: "0.2rem" }}>{job.title}</div>
                    <div style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))" }}>{job.employerName}</div>
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", marginBottom: "0.875rem" }}>
                  {job.milestones.map(m => (<div key={m.id} style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                    ...
                    background: m.status === "APPROVED" ? "hsl(145 65% 42% / 0.2)" : m.status === "IN_PROGRESS" ? "hsl(217 91% 55% / 0.2)" : "hsl(var(--surface-4))" }}>
                    ...
                    m.status === "IN_PROGRESS" ? <Clock size={10} color="hsl(217 91% 65%)"/> :
                      ...
                      <span style={{ fontSize: "0.78rem", fontWeight: 600, color: m.status === "APPROVED" ? "hsl(145 65% 50%)" : "hsl(220 15% 55%)" }}>{formatCurrency(m.amount, user?.currency || "NGN")}</span>
                    </div>))}
                </div>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.4rem" }}>
                    ...
                    <span style={{ fontSize: "0.72rem", fontWeight: 600, color: "hsl(var(--success))" }}>{formatCurrency(job.escrow.funded, user?.currency || "NGN")} / {formatCurrency(job.escrow.total, user?.currency || "NGN")}</span>
                  </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "0.875rem" }}>
                  <span style={{ fontSize: "0.75rem", color: "hsl(var(--text-3))" }}>Deadline: {formatDate(job.deadline)}</span>
            ...
            {activeJobs.length === 0 && (<div className="card" style={{ textAlign: "center", padding: "2rem" }}>
                ...
                <p style={{ color: "hsl(var(--text-3))", fontSize: "0.875rem" }}>No active contracts yet</p>
          ...
          {/* AI Match Banner */}
          <div style={{ marginTop: "1.25rem", background: "linear-gradient(135deg, hsl(217 91% 55% / 0.12), hsl(200 100% 60% / 0.06))", border: "1px solid hsl(var(--primary) / 0.3)", borderRadius: 12, padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "0.875rem", fontWeight: 700, color: "hsl(var(--text))" }}>3 new AI-matched jobs</div>
              <div style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))" }}>Based on your skills and delivery history. 94%+ match scores.</div>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.875rem" }}>
              <h2 className="font-heading" style={{ fontSize: "1rem", fontWeight: 700, color: "hsl(var(--text))" }}>Recent Payments</h2>
            ...
            <div className="card" style={{ padding: "0.75rem", display: "flex", flexDirection: "column", gap: "0" }}>
              {DEMO_TRANSACTIONS.slice(0, 3).map((tx, i) => (<div key={tx.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.625rem 0.5rem", borderBottom: i < 2 ? "1px solid hsl(var(--surface-3))" : "none" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: "0.8rem", fontWeight: 600, color: "hsl(var(--text))", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tx.type.replace(/_/g, " ")}</div>
                    <div style={{ fontSize: "0.72rem", color: "hsl(var(--text-2))" }}>{formatRelativeTime(tx.date)}</div>
                  </div>
                  <div style={{ fontSize: "0.875rem", fontWeight: 700, color: "hsl(var(--success))" }}>+{formatCurrency(tx.amount, user?.currency || "NGN")}</div>
                </div>))}
            </div>
          </div>
        ...
        ].map(({ label, value }) => (<div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "0.4rem 0", borderBottom: "1px solid hsl(var(--border))" }}>
                <span style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))" }}>{label}</span>
                <span style={{ fontSize: "0.78rem", fontWeight: 700, color: "hsl(var(--text))" }}>{value}</span>
              </div>))}
          </div>
        </div>
      </div>
