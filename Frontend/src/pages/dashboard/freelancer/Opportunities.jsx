...
export default function FreelancerOpportunitiesPage() {
    ...
    return (<div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: "2rem" }}>
        <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))" }}>Job Opportunities</h1>
        <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem", marginTop: 4 }}>AI-matched projects based on your skills</p>
      </div>

      {/* AI Match Banner */}
      <div style={{
            marginBottom: "2rem", background: "linear-gradient(135deg, hsl(217 91% 55% / 0.12), hsl(200 100% 60% / 0.06))",
            border: "1px solid hsl(var(--primary) / 0.3)", borderRadius: 12, padding: "1.25rem 1.5rem",
            display: "flex", alignItems: "center", gap: "1.25rem",
        }}>
        <div style={{ width: 44, height: 44, borderRadius: 11, background: "hsl(var(--primary) / 0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <Cpu size={22} color="hsl(217 91% 70%)"/>
        </div>
        <div>
          <div style={{ fontSize: "1rem", fontWeight: 700, color: "hsl(var(--text))", marginBottom: "0.2rem" }}>Gemini Semantic Matching Active</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        {openJobs.map((job) => {
          ...
          return (<div key={job.id} className="card card-hover" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <div style={{ flex: 1, paddingRight: "1rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
                    <h3 className="font-heading" style={{ fontSize: "1.1rem", fontWeight: 700, color: "hsl(var(--text))" }}>{job.title}</h3>
                    {job.isBackend && (
                      <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "hsl(var(--success))", background: "hsl(145 65% 42% / 0.12)", border: "1px solid hsl(145 65% 42% / 0.25)", padding: "0.1rem 0.35rem", borderRadius: 4, textTransform: "uppercase" }}>Live</span>
                    )}
                    <div style={{ display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.7rem", fontWeight: 700, color: "hsl(var(--primary-dark))", background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.3)", padding: "0.15rem 0.5rem", borderRadius: 4 }}>
                      <Cpu size={10}/> {90 + (Math.abs(job.title.length % 8))}% Match
                    </div>
                  </div>
                  <div style={{ fontSize: "0.83rem", color: "hsl(var(--text-2))", marginBottom: "1rem", lineHeight: 1.6 }}>
                    {job.description?.slice(0, 160)}{job.description?.length > 160 ? "…" : ""}
                  </div>
                  <div style={{ display: "flex", gap: "1rem", fontSize: "0.8rem", color: "hsl(var(--text-2))", marginBottom: "1rem", flexWrap: "wrap" }}>
                    <span>By: <strong style={{ color: "hsl(var(--text))" }}>{job.employer_name || job.employerName || "Afrizend Client"}</strong></span>
                  ...
                  <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                    {skillsArr.map((s) => (<span key={s} style={{ fontSize: "0.72rem", color: "hsl(var(--text-2))", background: "hsl(var(--surface-3))", padding: "0.2rem 0.5rem", borderRadius: 4 }}>{s}</span>))}
                  </div>
                </div>
              ...
              {/* Milestone preview */}
              {job.milestones?.length > 0 && (<div style={{ borderTop: "1px solid hsl(var(--border))", paddingTop: "0.875rem", display: "flex", gap: "0.35rem", flexWrap: "wrap", alignItems: "center" }}>
                  <span style={{ fontSize: "0.72rem", color: "hsl(var(--text-3))", marginRight: "0.25rem" }}>Milestones:</span>
                  {job.milestones.map((m) => (<span key={m.id} style={{ fontSize: "0.7rem", color: "hsl(var(--primary))", background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.3)", padding: "0.1rem 0.45rem", borderRadius: 4 }}>
        ...
        {openJobs.length === 0 && !loadingJobs && (<div className="card" style={{ textAlign: "center", padding: "3rem 1rem" }}>
            ...
            <h3 className="font-heading" style={{ fontSize: "1.1rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>No open jobs right now</h3>
            <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem" }}>Check back soon — new jobs are posted by employers daily.</p>
          </div>)}

        {loadingJobs && (<div style={{ textAlign: "center", padding: "2rem", color: "hsl(var(--text-2))", fontSize: "0.85rem" }}>
      ...
      {applyingJob && (<div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }} className="animate-fade-in">
          <div className="card glass-strong" style={{ width: "100%", maxWidth: 500, padding: "2rem", position: "relative" }}>
            <button onClick={() => setApplyingJob(null)} style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", cursor: "pointer", color: "hsl(var(--text-2))" }}>
              <X size={20}/>
            </button>
            ...
            {applySuccess ? (<div style={{ textAlign: "center", padding: "1.5rem 0" }}>
                ...
                <h3 className="font-heading" style={{ fontSize: "1.3rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>Application Submitted!</h3>
                ...
                {user && (
                  <div style={{ background: "hsl(var(--surface-3))", border: "1px solid hsl(var(--border))", borderRadius: 8, padding: "0.875rem", marginBottom: "1.25rem", display: "flex", gap: "0.75rem", alignItems: "center" }}>
                    <div className="avatar" style={{ width: 36, height: 36, fontSize: "0.75rem", borderRadius: 8, flexShrink: 0 }}>
                      {user.name?.split(" ").map(n => n[0]).join("") || "U"}
                    </div>
                    <div>
                      <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "hsl(var(--text))" }}>{user.name}</div>
                      <div style={{ fontSize: "0.72rem", color: "hsl(var(--text-2))" }}>
