...
function MilestoneStatusLabel({ status }) {
    const s = (status || '').toUpperCase();
    const map = {
        LOCKED: { label: "Locked", color: "hsl(var(--text-3))" },
    ...
    return (
        <div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
                <div>
                    <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))" }}>My Contracts</h1>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                {loading && (
                    <div style={{ textAlign: "center", padding: "3rem", color: "hsl(var(--text-2))", fontSize: "0.875rem" }}>
                ...
                {!loading && filtered.map((job) => {
                    ...
                    return (
                        <div key={job.id} className="card" style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                            ...
                            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                                <div style={{ flex: 1, paddingRight: "1rem" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
                                        <h3 className="font-heading" style={{ fontSize: "1.2rem", fontWeight: 700, color: "hsl(var(--text))" }}>{job.title}</h3>
                                        <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "hsl(var(--success))", background: "hsl(145 65% 42% / 0.12)", border: "1px solid hsl(145 65% 42% / 0.25)", padding: "0.1rem 0.4rem", borderRadius: 4, textTransform: "uppercase" }}>Live</span>
                                    </div>
                                    <div style={{ display: "flex", gap: "1rem", fontSize: "0.8rem", color: "hsl(var(--text-2))", flexWrap: "wrap" }}>
                                        <span>Employer: <strong style={{ color: "hsl(var(--text))" }}>{job.employer_name || "Client"}</strong></span>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.5rem" }}>
                                    <span style={{
                                        ...
                                        background: "hsl(var(--primary) / 0.1)", color: "hsl(var(--primary-dark))",
                                    }}>
                            ...
                            {/* Milestones */}
                            <div>
                                <h4 className="font-heading" style={{ fontSize: "1rem", color: "hsl(var(--text))", marginBottom: "1rem" }}>Milestones & Deliverables</h4>
                                    ...
                                    {milestones.map((m) => {
                                        ...
                                        return (
                                            <div key={m.id} style={{
                                                ...
                                                border: isPaid ? "1px solid hsl(145 65% 50% / 0.2)" : isApproved ? "1px solid hsl(38 92% 60% / 0.2)" : isActive ? "1px solid hsl(217 91% 55% / 0.2)" : isRevision ? "1px solid hsl(0 84% 60% / 0.2)" : "1px solid hsl(var(--surface-4))",
                                            }}>
