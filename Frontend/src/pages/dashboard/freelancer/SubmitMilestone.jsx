...
export default function SubmitMilestonePage() {
    ...
    const submitMilestone = useJobsStore((s) => s.submitMilestone);
    const recordVerdict = useJobsStore((s) => s.recordVerdict);
    const milestone = job?.milestones.find((m) => m.id === params.milestoneId);
    ...
    if (milestone.status !== "IN_PROGRESS" && milestone.status !== "NEEDS_REVISION") {
        ...
        <h2 className="font-heading" style={{ color: "white", marginBottom: "0.5rem" }}>
          {milestone.status === "APPROVED" ? "This milestone is already approved." : "This milestone is not ready for submission."}
        </h2>
        <p style={{ color: "hsl(220 15% 55%)", marginBottom: "1.5rem", fontSize: "0.875rem" }}>Status: {milestone.status}</p>
    ...
    async function handleSubmit() {
        ...
        submitMilestone(job.id, milestone.id, finalNotes, currentFiles);
        ...
        try {
            ...
            recordVerdict(job.id, milestone.id, result);
            setVerdict(result);
            setVerifyState("done");
        }
        catch (e) {
            setError("Verification failed: " + (e.message || "Check Gemini API key."));
      ...
      {verifyState === "idle" && (<div className="card" style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div style={{ background: "hsl(217 91% 55% / 0.06)", border: "1px solid hsl(217 91% 55% / 0.15)", borderRadius: 8, padding: "0.75rem 0.875rem", display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
            ...
            <p style={{ fontSize: "0.8rem", color: "hsl(220 15% 60%)", lineHeight: 1.5 }}>
              Gemini AI will review your submission against the acceptance criteria. Approved submissions release <strong style={{ color: "white" }}>{formatCurrency(milestone.amount)}</strong> instantly to your Kora wallet. Average review time: under 30 seconds.
            </p>
          </div>
