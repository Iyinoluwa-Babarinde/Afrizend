...
export default function SubmitMilestonePage() {
  ...
  if (loading) {
    return <div style={{ padding: "3rem", textAlign: "center", color: "hsl(var(--text-2))" }}>Loading milestone...</div>;
  }

  if (!job || !milestone) {
    return (<div style={{ padding: "3rem", textAlign: "center" }}>
      <p style={{ color: "hsl(var(--text-2))" }}>Milestone not found.</p>
  ...
  if (mStatus !== "IN_PROGRESS" && mStatus !== "NEEDS_REVISION") {
    return (<div style={{ padding: "3rem", textAlign: "center" }}>
      ...
      <h2 className="font-heading" style={{ color: "hsl(var(--text))", marginBottom: "0.5rem" }}>
        {mStatus === "APPROVED" || mStatus === "PAID" ? "This milestone is already approved." : "This milestone is not ready for submission."}
      </h2>
      <p style={{ color: "hsl(var(--text-2))", marginBottom: "1.5rem", fontSize: "0.875rem" }}>Status: {mStatus}</p>
  ...
  return (<div style={{ padding: "1.5rem", maxWidth: 700, margin: "0 auto" }}>
    <button onClick={() => navigate(-1)} style={{ display: "flex", alignItems: "center", gap: "0.4rem", background: "none", border: "none", color: "hsl(var(--text-2))", cursor: "pointer", fontSize: "0.875rem", marginBottom: "1.25rem", padding: 0 }}>
    ...
    <div style={{ marginBottom: "1.75rem" }}>
      ...
      <h1 className="font-heading" style={{ fontSize: "1.5rem", fontWeight: 800, color: "hsl(var(--text))", letterSpacing: "-0.03em" }}>
        {milestone.title}
      </h1>
    ...
    <div className="card" style={{ marginBottom: "1.5rem" }}>
      <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "hsl(var(--text))", marginBottom: "0.75rem" }}>What you need to deliver</h2>
      <p style={{ fontSize: "0.85rem", color: "hsl(var(--text-2))", lineHeight: 1.6, marginBottom: "1rem" }}>{milestone.description}</p>

      <div style={{ marginBottom: "1rem" }}>
        <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "hsl(var(--text-3))", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.5rem" }}>Required Deliverables</div>
        {milestone.deliverables.map((d, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.3rem" }}>
          ...
          <span style={{ fontSize: "0.82rem", color: "hsl(var(--text-2))" }}>{d}</span>
        </div>))}
      </div>
      ...
      <div style={{ background: "hsl(145 65% 42% / 0.06)", border: "1px solid hsl(145 65% 42% / 0.15)", borderRadius: 8, padding: "0.75rem 0.875rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", marginBottom: "0.3rem" }}>
          ...
          <span style={{ fontSize: "0.72rem", fontWeight: 700, color: "hsl(var(--success))", textTransform: "uppercase", letterSpacing: "0.05em" }}>AI Will Check</span>
        </div>
        <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", lineHeight: 1.5 }}>{milestone.acceptanceCriteria}</p>
      </div>
    </div>

    {/* Submission form */}
    {verifyState === "idle" && (<div className="card" style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "hsl(var(--text))" }}>Your Submission</h2>
      ...
      {milestone.status === "NEEDS_REVISION" && (<div style={{ background: "hsl(0 84% 60% / 0.08)", border: "1px solid hsl(0 84% 60% / 0.25)", borderRadius: 8, padding: "0.75rem", display: "flex", gap: "0.6rem" }}>
        ...
        <div>
          ...
          {milestone.verificationResult?.suggestedRevisions?.map((r, i) => (<div key={i} style={{ fontSize: "0.78rem", color: "hsl(var(--text-2))", marginBottom: "0.15rem" }}>• {r}</div>))}
        </div>
      </div>)}

      {/* Notes */}
      <div>
        <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "hsl(var(--text-2))", marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.05em" }}>
          Submission Notes *
        </label>
        ...
        <div style={{ fontSize: "0.72rem", color: "hsl(var(--text-3))", marginTop: "0.3rem" }}>
      ...
      <div>
        ...
        {files.length > 0 && (<div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
          {files.map((f) => (<span key={f} style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.78rem", color: "hsl(var(--primary-dark))", background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.3)", padding: "0.2rem 0.5rem", borderRadius: 4 }}>
            <FileText size={11} /> {f}
            <button onClick={() => setFiles(files.filter((x) => x !== f))} style={{ background: "none", border: "none", cursor: "pointer", color: "hsl(var(--text-3))", display: "flex", padding: 0 }}>
              <X size={11} />
            </button>
          </span>))}
        </div>)}
      ...
      <div style={{ background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.3)", borderRadius: 8, padding: "0.75rem 0.875rem", display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
        ...
        <p style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", lineHeight: 1.5 }}>
          Gemini AI will review your submission against the acceptance criteria. Approved submissions release <strong style={{ color: "hsl(var(--text))" }}>{formatCurrency(convertCurrency(milestone.amount, job.currency, user?.currency), user?.currency || 'NGN')}</strong> instantly to your KORA wallet. Average review time: under 30 seconds.
        </p>
      </div>
    ...
    {(verifyState === "uploading" || verifyState === "verifying") && (<div className="card" style={{ textAlign: "center", padding: "3rem 2rem" }}>
      <div style={{ width: 64, height: 64, borderRadius: "50%", background: "hsl(var(--primary) / 0.1)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.5rem", border: "1px solid hsl(var(--primary) / 0.3)" }}>
      ...
      <h2 className="font-heading" style={{ fontSize: "1.25rem", color: "hsl(var(--text))", marginBottom: "0.5rem" }}>
        {verifyState === "uploading" ? "Securing your submission..." : "Gemini is reviewing your work..."}
      </h2>
      <p style={{ color: "hsl(var(--text-2))", fontSize: "0.875rem", maxWidth: 360, margin: "0 auto" }}>
        {verifyState === "uploading"
      ...
      {verifyState === "verifying" && (<div style={{ marginTop: "1.5rem", display: "flex", gap: "0.5rem", justifyContent: "center", flexWrap: "wrap" }}>
        {["Reading submission notes", "Checking deliverables", "Evaluating criteria", "Computing verdict"].map((s, i) => (<span key={s} style={{ fontSize: "0.72rem", color: "hsl(217 91% 60%)", background: "hsl(var(--primary) / 0.1)", padding: "0.2rem 0.6rem", borderRadius: 4, border: "1px solid hsl(var(--primary) / 0.3)" }}>
          {s}
        </span>))}
      </div>)}
    </div>)}
    ...
    {verifyState === "done" && verdict && (<div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))", marginBottom: "0.75rem" }}>AI Confidence: {verdict.confidence}%</div>
        <p style={{ fontSize: "0.875rem", color: "hsl(var(--text-2))", lineHeight: 1.6, maxWidth: 480, margin: "0 auto" }}>{verdict.summary}</p>
      </div>
      ...
      {verdict.verdict === "APPROVED" && (<div style={{ background: "hsl(145 65% 42% / 0.06)", border: "1px solid hsl(145 65% 42% / 0.2)", borderRadius: 10, padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: "0.8rem", color: "hsl(var(--text-2))" }}>
            The employer will release payment from escrow. You'll receive it instantly to your KORA wallet.
          </div>
        </div>
      </div>)}
      ...
      {verdict.checklist?.length > 0 && (<div className="card">
        <h3 style={{ fontSize: "0.875rem", fontWeight: 700, color: "hsl(var(--text))", marginBottom: "0.875rem" }}>Verification Checklist</h3>
        {verdict.checklist.map((item, i) => (<div key={i} style={{ display: "flex", gap: "0.6rem", marginBottom: "0.6rem", padding: "0.5rem", background: "hsl(220 16% 10%)", borderRadius: 6 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "0.75rem", color: "hsl(var(--text-2))" }}>{item.note}</div>
          </div>
        </div>))}
      </div>)}
      ...
      {verdict.verdict === "NEEDS_REVISION" && verdict.suggestedRevisions && (<div className="card" style={{ background: "hsl(0 84% 60% / 0.04)", border: "1px solid hsl(0 84% 60% / 0.15)" }}>
        ...
        {verdict.suggestedRevisions.map((r, i) => (<div key={i} style={{ fontSize: "0.82rem", color: "hsl(var(--text-2))", padding: "0.4rem 0", borderBottom: i < verdict.suggestedRevisions.length - 1 ? "1px solid hsl(var(--surface-3))" : "none" }}>
          {i + 1}. {r}
        </div>))}
      </div>)}
    ...
    {verifyState === "error" && (<div className="card" style={{ textAlign: "center", padding: "2rem" }}>
      ...
      <h2 className="font-heading" style={{ color: "hsl(var(--text))", marginBottom: "0.5rem" }}>Verification Error</h2>
      <p style={{ color: "hsl(var(--text-2))", marginBottom: "1.5rem", fontSize: "0.875rem" }}>{error}</p>
